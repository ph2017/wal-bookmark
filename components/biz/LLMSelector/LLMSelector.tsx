...
const LLMSelector: React.FC<LLMSelectorProps> = ({
  ...
  const handleValueChange = (value: string) => {
    const [provider, model] = value.split(linkSymbol) as [AIProvider, string]
  ...
  const selectValue =
    selectedProvider && selectedModel
      ? `${selectedProvider}${linkSymbol}${selectedModel}`
      : undefined
            ...
            ([provider, models]) =>
              models.length > 0 && (
                <SelectGroup key={provider}>
                  ...
                  {models.map(model => (
                    <SelectItem
                      key={`${provider}${linkSymbol}${model.modelId}`}
                      value={`${provider}${linkSymbol}${model.modelId}`}
                    >
                      {model.title}
                    </SelectItem>
                  ))}
